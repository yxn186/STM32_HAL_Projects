...
/**
 * @brief BMI088 读ID TxRx回调函数
 * 
 * @param read_reg_address 读取寄存器地址
 * @param rx_data 接收数据地址
 * @param rx_length 接收数据长度
 */
static void bmi088_spi_txrx_readid_callback(uint8_t read_reg_address, uint8_t *rx_data, uint16_t rx_length)
{
...
void bmi088_readid_gyro(bmi088_handle_t *handle,bmi088_readid_gyro_finishedfunction readid_gyro_finishedfunction)
{
  ...
  bmi088_gyro_read_reg(handle,BMI088_GYRO_CHIP_ID,&handle->gyro_id,1,bmi088_spi_txrx_readid_callback);
}
